...
class ABCCommonAPITests(TestCase):
    ...
    def test_calc_expire_at_timestamp(self, mocked_get_now_datetime):
        ...
        val = dynamodb.ABCCommonAPI.calc_expire_at_timestamp(expire_in_days=1)
        self.assertEqual(val, 1062862408)

        val = dynamodb.ABCCommonAPI.calc_expire_at_timestamp(expire_in_days=180)
        self.assertEqual(val, 1078328008)

        val = dynamodb.ABCCommonAPI.calc_expire_at_timestamp(expire_in_days=None)
